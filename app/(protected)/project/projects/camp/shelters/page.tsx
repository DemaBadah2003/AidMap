...
const sel = 'w-full rounded-lg border border-input bg-background px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500'
...
function SheltersTab() {
  ...
  const validate = (d: typeof formData) => {
    ...
    if (!d.phone.trim() || !/^(056|059)\d{7}$/.test(d.phone.trim())) e.phone = 'رقم غير صحيح (056 أو 059 + 7 أرقام)'
    if (!d.capacity || d.capacity <= 0) e.capacity = 'يجب أن يكون أكبر من 0'
    setErrors(e)
    ...
    <div>
      <Card className="overflow-hidden border shadow-sm rounded-xl">
        <CardContent className="p-0">
          <div className="p-4 flex flex-col sm:flex-row items-center gap-3 border-b">
            ...
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as any)} className={`${sel} h-10 w-auto min-w-[130px]`}>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="max-w-md rounded-2xl" dir="rtl">
          <DialogHeader>
            <DialogTitle className="text-right text-lg font-bold text-blue-800">إضافة مركز إيواء</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={!!editItem} onOpenChange={open => { if (!open) setEditItem(null) }}>
        <DialogContent className="max-w-md rounded-2xl" dir="rtl">
          <DialogHeader>
            <DialogTitle className="text-right text-lg font-bold text-blue-800">تعديل مركز إيواء</DialogTitle>
          </DialogHeader>
          <ShelterForm formData={formData} setFormData={setFormData} errors={errors} />
          {errors._ && <p className="text-sm text-red-600">{errors._}</p>}
          <DialogFooter className="mt-2 flex flex-col sm:flex-row gap-3">
...
function ShelterForm({
  ...
  formData: { nameAr: string; areaAr: string; supervisorAr: string; phone: string; capacity: number; fillStatus: 'ممتلئ' | 'غير ممتلئ' }
  ...
  return (
    <div className="flex flex-col gap-4 py-2 text-right">
      ...
      <div className="space-y-1.5">
        ...
        <select className={`${sel} h-11`} value={formData.areaAr} onChange={e => setFormData({ ...formData, areaAr: e.target.value })}>
      ...
      <div className="space-y-1.5">
        ...
        {errors.supervisorAr && <p className="text-xs text-red-500">{errors.supervisorAr}</p>}
      </div>
      <div className="space-y-1.5">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1.5">
          ...
          <Input type="number" min="0" className={`h-11 ${errors.capacity ? 'border-red-400' : ''}`} value={formData.capacity || ''} onChange={e => setFormData({ ...formData, capacity: Number(e.target.value) })} />
        ...
        <div className="space-y-1.5">
          ...
          <select className={`${sel} h-11`} value={formData.fillStatus} onChange={e => setFormData({ ...formData, fillStatus: e.target.value })}>
            <option value="غير ممتلئ">غير ممتلئ</option>
            <option value="ممتلئ">ممتلئ</option>
          </select>
        </div>
...
function SupervisorsTab() {
  ...
  const isValidPhone = (p: string) => /^(056|059)\d{7}$/.test(p)

  const onAdd = async () => {
    if (!addForm.nameAr.trim() || !isValidPhone(addForm.phone)) { setAddError('تأكد من إدخال الاسم ورقم جوال صحيح (056/059 + 7 أرقام)'); return }
    setSubmitting(true)
    setAddError('')
    try {
      const res = await fetch(SUPERVISOR_API, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nameAr: addForm.nameAr, phone: addForm.phone, region: addForm.region, status: addForm.status }) })
      const data = await res.json()
      if (!res.ok) { setAddError(data.message ?? 'خطأ'); return }
      setItems(prev => [data, ...prev])
      setAddOpen(false)
      setAddForm({ nameAr: '', phone: '', status: 'نشط' })
    } finally { setSubmitting(false) }
  }

  const onEdit = async (id: string) => {
    if (!editDraft.nameAr.trim() || !isValidPhone(editDraft.phone)) return
    setSubmitting(true)
    try {
      const res = await fetch(`${SUPERVISOR_API}?id=${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nameAr: editDraft.nameAr, phone: editDraft.phone, status: editDraft.status }) })
      const data = await res.json()
      if (res.ok) {
    ...
    <div>
      <Card className="overflow-hidden border shadow-sm rounded-xl">
        <CardContent className="p-0">
          <div className="p-4 flex flex-col sm:flex-row items-center gap-3 border-b">
            ...
            <Button onClick={() => { setAddForm({ nameAr: '', phone: '', region: '', status: 'نشط' }); setAddError(''); setAddOpen(true) }} className="bg-blue-600 text-white hover:bg-blue-700 h-10 px-4 gap-2 sm:mr-auto">
            ...
            <table className="w-full text-right border-collapse text-sm">
              ...
              <tbody className="divide-y">
                {loading ? (
                  ...
                  <tr key={sp.id} className={`hover:bg-muted/30 transition-colors ${editingId === sp.id ? 'bg-blue-50/30' : ''}`}>
                    <td className="p-4 font-bold text-foreground">
                      {editingId === sp.id
                        ? <Input value={editDraft.nameAr} onChange={e => setEditDraft(d => ({ ...d, nameAr: e.target.value }))} className="h-9" />
                        : sp.nameAr}
                    </td>
                    <td className="p-4 text-muted-foreground" dir="ltr">
                      {editingId === sp.id
                        ? <Input value={editDraft.phone} onChange={e => setEditDraft(d => ({ ...d, phone: e.target.value }))} className="h-9" maxLength={10} />
                        : sp.phone}
                    </td>
                    <td className="p-4">
                      {editingId === sp.id
                        ? <select value={editDraft.status} onChange={e => setEditDraft(d => ({ ...d, status: e.target.value as 'نشط' | 'موقوف' }))} className={`${sel} h-9`}>
                    ...
                    <td className="p-4 text-center">
                      {editingId === sp.id ? (
                        <div className="flex gap-2 justify-center">
                          <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white h-8 w-8 p-0 rounded-full" onClick={() => onEdit(sp.id)} disabled={submitting}>
                            {submitting ? <Loader2 className="h-3 w-3 animate-spin" /> : <Save className="h-4 w-4" />}
                          </Button>
                          <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-red-400 hover:bg-red-50 rounded-full" onClick={() => setEditingId(null)}>
                            <X className="h-4 w-4" />
                          </Button>
                        </div>
                      ) : (
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="max-w-sm rounded-2xl" dir="rtl">
          <DialogHeader>
            <DialogTitle className="text-right text-lg font-bold text-blue-800">إضافة مشرف</DialogTitle>
          </DialogHeader>
          <div className="flex flex-col gap-4 py-2 text-right">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-foreground">الاسم *</label>
              <Input className="h-11" value={addForm.nameAr} onChange={e => setAddForm(f => ({ ...f, nameAr: e.target.value }))} />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-foreground">الهاتف *</label>
              <Input className="h-11" value={addForm.phone} onChange={e => setAddForm(f => ({ ...f, phone: e.target.value }))} placeholder="059xxxxxxx" dir="ltr" maxLength={10} />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-foreground">المنطقة</label>
              <select className={`${sel} h-11`} value={addForm.region} onChange={e => setAddForm(f => ({ ...f, region: e.target.value }))}>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-foreground">الحالة</label>
              <select className={`${sel} h-11`} value={addForm.status} onChange={e => setAddForm(f => ({ ...f, status: e.target.value as 'نشط' | 'موقوف' }))}>
                <option value="نشط">نشط</option>
                <option value="موقوف">موقوف</option>
              </select>
            </div>
    ...
    <div className="w-full px-4 py-6" dir="rtl">
      ...
      <div className="flex gap-2 mb-6 border-b pb-0">
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-t-lg border-b-2 transition-colors ${tab === 'shelters' ? 'border-blue-600 text-blue-700 bg-blue-50' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
        >
          <Home className="w-4 h-4" /> مراكز الإيواء
        </button>
        <button
          onClick={() => setTab('supervisors')}
          className={`flex items-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-t-lg border-b-2 transition-colors ${tab === 'supervisors' ? 'border-blue-600 text-blue-700 bg-blue-50' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
        >
          <UserCheck className="w-4 h-4" /> المشرفون
        </button>
      </div>
