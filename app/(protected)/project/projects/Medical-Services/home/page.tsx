...
export default function AdvancedReliefSystem() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}?type=locations`)
      .then(res => res.json())
      .then(data => {
        // نتحقق أن البيانات مصفوفة فعلاً قبل التحديث
        if (Array.isArray(data)) {
          setDbLocations(data);
        } else {
          console.error("Data received is not an array:", data);
          setDbLocations([]); // نرجعها مصفوفة فارغة لتجنب الـ Error
        }
      })
      .catch(err => {
  ...
  useEffect(() => {
    if (selectedLoc) {
      fetch(`${API_BASE}?type=regions&location=${selectedLoc}`)
        .then(res => res.json())
        .then(data => setDbRegions(Array.isArray(data) ? data : []))
    }
  }, [selectedLoc])
  ...
  useEffect(() => {
    if (selectedReg) {
      fetch(`${API_BASE}?type=hospitals&region=${selectedReg}`)
        .then(res => res.json())
        .then(data => setDbHospitals(Array.isArray(data) ? data : []))
    }
  }, [selectedReg])
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                {/* استخدام الاختصار (dbLocations || []) لضمان أنها مصفوفة */}
