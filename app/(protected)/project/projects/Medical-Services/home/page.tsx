  ...
  return (
    <div className="w-full bg-[#f8fafc] font-arabic antialiased h-screen overflow-y-auto scroll-smooth pb-20" dir="rtl">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        {/* 1. العنوان الرئيسي فوق الكارد */}
        <div className="text-right pt-12 mb-8 space-y-6">
          <div className="flex items-center gap-4 justify-center md:justify-start">
            <div className="bg-blue-600 p-2.5 rounded-2xl text-white shadow-lg"><Activity className="w-8 h-8" /></div>
            <h1 className="text-[#1e3a8a] text-3xl md:text-4xl font-black italic">البحث الذكي عن النقاط الطبية</h1>
          </div>
        </div>

        {/* 2. كارد البحث - الأزرار والمدخلات في صف واحد */}
        <Card className="border-none shadow-sm rounded-[25px] bg-white mb-10 overflow-visible border border-slate-50">
          <CardContent className="p-5">
            <div className="flex flex-wrap md:flex-nowrap items-end gap-4 justify-center">
              
              {/* اختيار المنطقة */}
              <div className="flex-1 min-w-[180px] space-y-1">
                <label className="text-[10px] font-bold text-slate-400 flex items-center gap-2 mr-2">
                  <MapPin className="w-3 h-3 text-blue-500" /> المنطقة
                </label>
                <select 
                  className="w-full h-12 bg-[#f8fafc] border border-slate-100 rounded-xl px-4 text-right outline-none focus:border-blue-300 transition-all cursor-pointer text-sm"
                  value={selectedLoc} 
              ...
              <div className="flex-1 min-w-[220px] space-y-1">
                <label className="text-[10px] font-bold text-slate-400 flex items-center gap-2 mr-2">
                  <Building2 className="w-3 h-3 text-blue-500" /> المنشأة
                </label>
                <select 
                  className="w-full h-12 bg-[#f8fafc] border border-slate-100 rounded-xl px-4 text-right outline-none focus:border-blue-300 transition-all cursor-pointer text-sm"
                  value={selectedHosp} 
              ...
              {/* البحث عن طبيب */}
              <div className="flex-[1.5] min-w-[250px] space-y-1">
                <label className="text-[10px] font-bold text-slate-400 flex items-center gap-2 mr-2">
                  <User className="w-3 h-3 text-blue-500" /> ابحث عن طبيب (اختياري)
                </label>
                <input 
                  type="text" 
                  placeholder="اسم الطبيب أو التخصص..." 
                  className="w-full h-12 bg-[#f8fafc] border border-slate-100 rounded-xl px-4 text-right outline-none focus:ring-2 focus:ring-blue-50 text-sm placeholder:text-slate-300"
                  value={doctorSearch}
                  onChange={(e) => setDoctorSearch(e.target.value)}
                />
              </div>

              {/* زر البحث الصغير بجانبهم */}
              <button 
                onClick={handleSearch} 
                className="bg-blue-600 hover:bg-blue-700 text-white font-bold h-12 px-8 rounded-xl shadow-md flex items-center justify-center gap-2 transition-all active:scale-95 group mb-[1px]"
              >
        ...
        {/* 3. نتائج البحث (تظهر فقط عند الضغط على بحث) */}
        {showResults && (
