...
export default function AdvancedQueryPage() {
  ...
  // Fetch hospitals when region changes
  useEffect(() => {
    if (!selectedRegion) { setHospitals([]); return }
  ...
  useEffect(() => {
    if (!selectedHospitalId) { setDepartments([]); return }
  ...
  // Fetch services + doctors when department changes
  useEffect(() => {
    if (!selectedDepartmentId) { setServices([]); setDoctors([]); return }
  ...
  return (
    <div className="w-full px-4 py-8 sm:px-10">
        ...
        <CardContent className="p-8 bg-slate-50/30">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              ...
              <select
                className={selectClass}
                disabled={!selectedRegion || loadingHospitals}
              ...
              {selectedRegion && !loadingHospitals && hospitals.length === 0 && (
                <p className="text-xs text-slate-400">لا توجد مستشفيات في هذه المنطقة</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {selectedHospitalId && !loadingDepartments && departments.length === 0 && (
                <p className="text-xs text-slate-400">لا توجد أقسام مسجلة لهذا المستشفى</p>
              )}
            </div>
          </div>
        </CardContent>
      ...
      {/* Loading detail */}
      {selectedDepartmentId && loadingDetail && (
            ...
            {services.length === 0 ? (
              ...
              <div className="grid grid-cols-2 gap-3">
                {services.map(s => (
                  <div
                    key={s.id}
                    className={`p-3 rounded-xl text-center font-bold border text-sm ${
                      s.isAvailable
                  ...
                  >
                    {s.name}
                    {s.price > 0 && (
                      <span className="block text-xs font-normal mt-1 opacity-70">{s.price} ₪</span>
                    )}
                  </div>
                ))}
              </div>
      ...
      {selectedHospitalId && !selectedDepartmentId && !loadingDepartments && departments.length > 0 && (
