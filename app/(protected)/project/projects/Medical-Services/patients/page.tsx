...
export default function PatientsPage() {
  ...
  const validate = (formData: any, currentId: string | null = null) => {
    ...
    if (!/^\d{9}$/.test(formData.nationalId)) {
      ...
      if (isIdExists) newErrors.nationalId = "رقم الهوية هذا مسجل لمريض آخر"
    }
    ...
    if (!formData.phone || !phoneRegex.test(formData.phone)) {
      newErrors.phone = "يجب أن يبدأ بـ 056 أو 059 متبوعاً بـ 7 أرقام"
    } else {
      const isPhoneExists = items.some(p => p.phone === formData.phone && p.id !== currentId)
      if (isPhoneExists) newErrors.phone = "رقم الهاتف مستخدم مسبقاً"
    }

    const ageNum = Number(formData.age)
    if (!formData.age || isNaN(ageNum) || ageNum < 1 || ageNum > 150) {
      newErrors.age = "العمر بين 1 و 150"
    }
  ...
  const onAdd = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert("فشل الحفظ، ربما البيانات مكررة في السيرفر")
      }
  ...
  const handleUpdate = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) { 
        ...
        alert("فشل التحديث، يرجى التأكد من عدم تكرار البيانات")
      }
    } catch (err) { alert("خطأ اتصال") }
    finally { setSubmitting(false) }
  }
  ...
  return (
    <div className="w-full px-2 md:px-4 py-6 font-sans text-right" dir="rtl">
      <div className="mb-6">
      ...
      <Card className="rounded-xl border-slate-200 shadow-sm bg-white overflow-hidden flex flex-col">
        <CardContent className="p-0">
          <div className="p-4 border-b bg-slate-50 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <Button onClick={() => { setErrors({}); setAddOpen(true) }} className="w-full md:w-auto bg-blue-600 hover:bg-blue-700">
              <Plus className="ml-2 h-4 w-4" /> إضافة مريض جديد
            </Button>
          </div>

          {/* الجزء المعدل: إضافة السكرول هنا */}
          <div className="overflow-x-auto overflow-y-auto max-h-[450px]">
            <table className="w-full text-sm min-w-[900px] border-collapse">
              <thead className="bg-slate-100 border-b font-bold text-slate-600 sticky top-0 z-10 shadow-sm">
                <tr>
                  ...
                  <th className="p-4 text-right w-24">العمر</th>
        ...
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto" dir="rtl">
          ...
          <div className="flex flex-col gap-4 py-4 text-right">
            <div>
              ...
              <Input className={errors.name ? "border-red-500" : ""} placeholder="أدخل الاسم الرباعي كاملاً" value={addForm.name} onChange={e => setAddForm({ ...addForm, name: e.target.value })} />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input type="number" className={errors.age ? "border-red-500" : ""} placeholder="1-150" value={addForm.age} onChange={e => setAddForm({ ...addForm, age: e.target.value })} />
