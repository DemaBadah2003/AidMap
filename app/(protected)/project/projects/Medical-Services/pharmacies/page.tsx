...
const BASE_URL = '/api/project/projects/pharmacies' 
...
export default function PharmaciesPage() {
  ...
  // States للإضافة (جدول الصيدليات)
  const [pharmacyName, setPharmacyName] = useState('')
  const [phone, setPhone] = useState('')
  const [medicines, setMedicines] = useState('') // ستخزن "متاح" أو "غير متاح"
  
  const [submitting, setSubmitting] = useState(false)
  const [editingId, setEditingId] = useState<string | null>(null)
  const [editDraft, setEditDraft] = useState<Omit<Pharmacy, 'id'>>({
    pharmacyName: '', phone: '', medicines: ''
  })
  ...
  const isEditValid = useMemo(() => {
    return editDraft.pharmacyName.trim() !== '' && editDraft.phone.trim() !== '' && editDraft.medicines !== ''
  }, [editDraft])

  const isAddValid = useMemo(() => {
    return pharmacyName.trim() !== '' && phone.trim() !== '' && medicines !== ''
  }, [pharmacyName, phone, medicines])
  ...
  const onAdd = async () => {
    ...
    try {
      const res = await fetch(BASE_URL, {
        ...
        body: JSON.stringify({ pharmacyName, phone, medicines })
      })
      if (res.ok) {
        await fetchData()
        setAddOpen(false)
  ...
  const resetAddForm = () => {
    setPharmacyName(''); setPhone(''); setMedicines('');
  }

  const filtered = useMemo(() => {
    return items.filter((c) => {
      return !q || c.pharmacyName.includes(q) || (c.phone || '').includes(q)
    })
  }, [q, items])
  ...
  return (
    <div className="w-full px-4 py-6" dir="rtl">
      <div className="mb-6 text-right font-arabic">
        <h1 className="text-2xl font-bold text-slate-900">إدارة الصيدليات</h1>
      </div>
        ...
        <CardContent className="p-0">
          <div className="p-4 flex flex-col sm:flex-row items-center gap-3 border-b">
            ...
            <Button onClick={() => setAddOpen(true)} className={`bg-blue-600 text-white hover:bg-blue-700 ${topControlHeight} px-4 rounded-lg mr-auto font-bold`}>
              <Plus className="h-4 w-4 ml-2" /> إضافة صيدلية
            </Button>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-right border-collapse text-sm">
              <thead className="bg-slate-50/80 border-b border-slate-100">
                <tr>
                  <th className="p-4 text-slate-500 font-bold">اسم الصيدلية</th>
                  <th className="p-4 text-slate-500 font-bold">رقم الهاتف</th>
                  <th className="p-4 text-slate-500 font-bold">حالة الأدوية</th>
                ...
                {loading ? (
                  ...
                  return (
                    <tr key={c.id} className={isEditing ? 'bg-blue-50/40' : 'hover:bg-slate-50/50 transition-colors'}>
                      <td className="p-4 font-bold">{isEditing ? <Input value={editDraft.pharmacyName} onChange={e => setEditDraft({...editDraft, pharmacyName: e.target.value})} /> : c.pharmacyName}</td>
                      <td className="p-4">{isEditing ? <Input value={editDraft.phone} onChange={e => setEditDraft({...editDraft, phone: e.target.value})} /> : c.phone}</td>
                      <td className="p-4">
                        {isEditing ? (
                          <select className={selectBaseClass} value={editDraft.medicines} onChange={e => setEditDraft({...editDraft, medicines: e.target.value})}>
                             <option value="">اختر الحالة</option>
                             <option value="متاح">متاح</option>
                             <option value="غير متاح">غير متاح</option>
                          </select>
                        ) : (
                          <span className={`px-2 py-1 rounded-full text-xs font-bold ${c.medicines === 'متاح' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                            {c.medicines || '-'}
                          </span>
                        )}
                      </td>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent dir="rtl" className="max-w-md font-arabic rounded-2xl shadow-2xl border-none">
          <DialogHeader>
            <DialogTitle className="text-right text-lg font-bold">إضافة صيدلية جديدة</DialogTitle>
          </DialogHeader>
          <div className="flex flex-col gap-5 py-4 text-right">
            
            {/* 1. اسم الصيدلية */}
            <div className="space-y-1.5">
               <label className="text-xs font-bold text-slate-700">اسم الصيدلية</label>
               <Input className="h-11 bg-slate-50 border-slate-200" value={pharmacyName} onChange={e => setPharmacyName(e.target.value)} placeholder="أدخل اسم الصيدلية" />
            </div>

            {/* 2. رقم الهاتف */}
            <div className="space-y-1.5">
               <label className="text-xs font-bold text-slate-700">رقم الهاتف للصيدلية</label>
               <Input className="h-11 bg-slate-50 border-slate-200" value={phone} onChange={e => setPhone(e.target.value)} placeholder="05XXXXXXXX" />
            </div>

            {/* 3. حالة الأدوية (Droplist) */}
            <div className="space-y-1.5">
               <label className="text-xs font-bold text-slate-700">الأدوية المتوفرة والغير متوفرة</label>
               <select 
