...
export default function PublicLayout({
  ...
  return (
    <div
      dir="rtl"
      className="flex h-full min-h-0 w-full flex-col overflow-hidden bg-slate-100 text-slate-800 m-0 p-0"
    >
      <header className="sticky top-0 z-50 h-16 shrink-0 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="flex h-full w-full items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden items-center gap-5 text-sm font-medium lg:flex">
            ...
            <Link
              href="/users/RegisterBeneficiary"
              className="text-slate-600 hover:text-blue-600"
            >
              تسجيل مستفيد
            </Link>
      ...
      <main className="flex-1 min-h-0 overflow-hidden m-0 p-0">
        <div className="h-full w-full min-h-0 overflow-hidden m-0 p-0">
          {children}
        </div>
      </main>
    </div>
